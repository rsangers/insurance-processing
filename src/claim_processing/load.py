import os
from claim_processing.pydantic_models import Document
from claim_processing.utils import extract_text_from_image
from claim_processing.pydantic_models import Claim
from claim_processing.constants import CLAIM_DIRECTORY, POLICY_DIRECTORY
    ...
    for file_name in os.listdir(claim_directory):
        ...
        if file_name.endswith(".md"):
            with open(file_path, "r") as file:    
                supporting_documents.append(Document(name=file_name, content=file.read(), type="supporting document"))
        elif file_name.endswith((".png", ".jpg", ".webp")):
            text = extract_text_from_image(file_path)
            supporting_documents.append(Document(name=file_name, content=text, type="supporting document"))
        else:
...
def load_claim(claim_id: str):
    ...
    return Claim(claim_id=claim_id, description=description, supporting_documents=supporting_documents)
